...
public class Statistics {
	...
	public static int updateUserStatistics(int userId, double newGameWinnings,
			boolean gameWon) throws SQLException {
		int updated = -1;
	...
	/**
	 * This generates an array of Strings that is the leaderboard of top 25,
	 * ordered by game winnings
	 * 
	 * @return
	 * @author Peter
	 */
	public static String[][] createLeaderBoard() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				leaderboard[i][0] = UserData.getUserObject(
						Integer.parseInt(rs.getString("u_id"))).getName();
